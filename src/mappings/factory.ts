...
export function handleFeePercentageChange(event: FeeChangedEvent): void {
  ...
  entity.feePenality = event.params.feePenalty
...
export function handleOwnershipTransferStarted(event: OwnershipTransferStartedEvent): void {
  const entity = new OwnershipTransferStarted(event.transaction.hash.concatI32(event.logIndex.toI32()))
...
export function handleRaffleCreated(event: RaffleCreatedEvent): void {
  ...
  raffle.state = raffleContract.state()
